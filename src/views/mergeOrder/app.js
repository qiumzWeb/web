...
import { getSearch, getSubmit, getEyeVideoConfig, getMorePackage, getConfirmCancel, getNotifyServer, getLessCode, getSplitCode, getVolumeTagPrint } from './api'
...
export default function App(props) {
    ...
    const [getScanStatus, setScanStatus] = useAction('mergeScanFlag')
    const [getCartCode, setCartCode] = useAction('mergeCode') // 合箱框号
    const [getTagCode, setTagCode] = useAction('mergeTagCode') // 合箱小票单号
    const [getDialogStatus, setDialogStatus] = useAction('mergeDialogFlag')
    // 获取操作人工号
    const [getOperatorCode, setOperatorCode] = useAction('UppackagingOperatorCode');
    ...
    // 合箱重量累加值
    const [getMergeWeightTotal, setMergeWeightTotal] = useAction('mergeWeightTotal');
...
// 判断是否自动提交
    function isAutoSubmit() {
      if (isUnUseAutoSubmit) return false;
            ...
            if (isSame(d.deliveryCode, value)) {
              if (d.isActive == 1) {
                ...
                Message.success(<div style={{display: 'flex', justifyContent: 'center',flexDirection: 'column'}}>
                  ...
                  {isCHOICEPackage(d) && <div>
                    {value}
                    <div>汇单包裹</div>
                    <div  style={{fontSize: px2rem(60)}}>请拆一段包裹</div>
                  </div>}
                </div>)
                // 需要拆包包裹语音提示
                if (isCHOICEPackage(d)) {
                  let a = setTimeout(() => {
                    POWWT.speakText("请拆包");
                    clearTimeout(a)
                  }, 500)
...
// 包裹框号扫描
    ...
    const getList = async (data) => {
        ...
        getFocus('searchCode')
        // 扫码提交直接提交
        if (value.toLowerCase() === 'scansubmit') {
          if (checkPackage('nomsg')) {
            submit();
          }
          return;
        }
...
// 自动提交
    function getAutoSubmit() {
      if (checkPackage('nomsg')) {
        if (isUNAutoSubmit && !isAutoSubmit()) return;
        submit()
      }
    }
// 提交合箱
...
// 提交拆单
    ...
    return <div className="mergeOrder">
        ...
        <div slot="btnTools" style={{ display: 'flex', justifyContent: 'space-between', marginTop: '15px' }}>
            ...
            <div>
                {!isUnUseAutoSubmit && <AutoSubmit autoKey={autoKey}></AutoSubmit>}
